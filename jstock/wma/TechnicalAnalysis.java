...
public class TechnicalAnalysis {
    ...
    public static Double createEMA(java.util.List<Double> values, int period) {
     ...
     /**
     * Returns the latest WMA.
     *
     * @param values list of raw data input
     * @param period the duration period
     * @return the latest WMA
     */
    public static Double createWMA(java.util.List<Double> values, int period) {
        if (period <= 0) {
            throw new java.lang.IllegalArgumentException("period must be greater than 0");
        }
        final int size = values.size();
        final Core core = new Core();
        final int allocationSize = size - core.wmaLookback(period);
        if (allocationSize <= 0) {
            return null;
        }
    ...
    public static TimeSeries createEMA(List<ChartData> chartDatas, String name, int period) {
     ...
     /**
     * Returns WMA time series for charting purpose.
     *
     * @param chartDatas list of chart data
     * @param name name for the time series
     * @param period the duration period
     * @return WMA time series for charting purpose
     */
    public static TimeSeries createWMA(List<ChartData> chartDatas, String name, int period) {
        if (period <= 0) {
            throw new java.lang.IllegalArgumentException("period must be greater than 0");
        }
        ...
        final Core core = new Core();
        final int allocationSize = num - core.wmaLookback(period);
        if (allocationSize <= 0) {
            return series;
        }
        final double[] last = new double[num];
