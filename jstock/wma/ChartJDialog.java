...
public class ChartJDialog extends javax.swing.JDialog {
    ...
    private void buildTAMenuItems() {
        ...
        final String[] ta_keys = {"ChartJDialog_SMA", "ChartJDialog_EMA", "ChartJDialog_WMA", "ChartJDialog_MACD", "ChartJDialog_RSI", "ChartJDialog_MFI", "ChartJDialog_CCI" };
        final String[] ta_tip_keys = {"ChartJDialog_SimpleMovingAverage", "ChartJDialog_ExponentialMovingAverage", "ChartJDialog_WeightMovingAverage", "ChartJDialog_MovingAverageConvergenceDivergence", "ChartJDialog_RelativeStrengthIndex", "ChartJDialog_MoneyFlowIndex", "ChartJDialog_CommodityChannelIndex" };
        final String[] custom_message_keys = {
            ...
            "info_message_please_enter_number_of_days_for_WMA",
            "dummy",
                    ...
                    item.addActionListener(new java.awt.event.ActionListener() {
                        ...
                        public void actionPerformed(java.awt.event.ActionEvent evt) {
                            if (ta == TA.SMA) {
                                ...
                                updateWMA(days[_j], item.isSelected());
                            }
                            else if (ta == TA.MFI) {
    ...
    private void clearAll() {
        for (int i = 0; i < this.activeTAExs.size(); i++) {
            ...
            else if (taEx.ta == TA.WMA) {
                this.updateWMA((Integer)taEx.parameter, false);
                i--;
            }
    ...
    private String getWMAKey(int days) {
        Interval interval = this.getCurrentInterval();
        String c = "d";
        if (interval == Interval.Weekly) {
            c = "w";
    ...
    private void updateWMA(int days, boolean show) {
        if (this.priceVolumeChart == null) {
            this.priceVolumeChart = this.createPriceVolumeChart(this.priceDataset, this.volumeDataset);
        }
        if (this.candlestickChart == null) {
        ...
        final TAEx taEx = TAEx.newInstance(TA.WMA, new Integer(days));
        if (show) {
            TimeSeries timeSeries = null;
            XYDataset dataSet = null;
            final Integer days_integer = days;
            if (false == time_series_weight_moving_average_map.containsKey(days_integer)) {
                timeSeries = TechnicalAnalysis.createWMA(this.chartDatas, this.getWMAKey(days), days);
                dataSet = new TimeSeriesCollection(timeSeries);
                // Do not put everything into map. We will out of memory.
                if (this.time_series_weight_moving_average_map.size() < MAX_MAP_SIZE) {
                    time_series_weight_moving_average_map.put(days_integer, timeSeries);
                }
            }
            else {
                timeSeries = time_series_weight_moving_average_map.get(days_integer);
                dataSet =  new TimeSeriesCollection(timeSeries);
            }
            ...
            {
                final TimeSeries ts = ((TimeSeriesCollection)this.priceDataset).getSeries(this.getWMAKey(days));
                if (ts != null) {
                    ((TimeSeriesCollection)this.priceDataset).removeSeries(ts);
                }
            }
    ...
    private final Map<Integer, TimeSeries> time_series_weight_moving_average_map = new HashMap<Integer, TimeSeries>();
